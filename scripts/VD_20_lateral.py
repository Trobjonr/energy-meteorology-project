...
print("Loading Armadillo Flats data (Sept 20)...")
df_list = []
for f in FILES:
    if os.path.exists(f):
        tmp = pd.read_csv(f)
...
# ==========================================================
# 2. TIME FILTER (UNSTABLE PHASE > 08:30 CDT)
# ==========================================================
# Adjust this manually if you want the full flight or stable phase
min_hr = df['Hour'].min()
if min_hr >= 12:
    print("  -> Detected UTC Timezone.")
    START_HOUR, START_MIN = 13, 00  # 09:00 CDT
else:
    print("  -> Detected Local Time (CDT).")
    START_HOUR, START_MIN = 9, 00

start_time = datetime.time(START_HOUR, START_MIN, 0)
print(f"Applying Time Filter: Keeping data AFTER {start_time}...")
...
if results:
    ...
    plt.title(f"Wake Recovery: Sept 20 (> 08:30 CDT)\nLocalized Reference Method", fontweight='bold')
    ...
    out_name = "Armadillo_Recovery_Sept20_Localized.png"
